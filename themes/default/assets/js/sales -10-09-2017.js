			...
			if(item.options !== false) {
				...
				$.each(item.options, function () {
					if(o == 1) {
						...
						$('#piece').val((item.row.piece ? item.row.piece : 0));
	...
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			var product_id = item.row.id,
				...
				cur_stock_qty = item.row.cur_stock_qty,
			...
			if (item_promotion == 1 && (current_date >= start_date && current_date <= end_date)){
				tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatMoney(real_unit_price) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + formatMoney(real_unit_price) + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + formatMoney(real_unit_price) + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(real_unit_price) + '</span></td>';
			}else{
				if (owner || admin || sale_price) {
					tr_html += '<td class="text-right"><input class="form-control text-right rprice_t" name="net_price[]" type="text" id="price_' + row_no + '" value="' + formatMoney(real_unit_price) + '"><input class="item_cost" name="item_cost[]" type="hidden" value="' + item_cost + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + formatDecimal(real_unit_price) + '"> </td>';
				} else {
					tr_html += '<input class="form-control text-right rprice_t" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatMoney(real_unit_price) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="item_cost" name="item_cost[]" type="hidden" value="' + item_cost + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + formatDecimal(real_unit_price) + '">';
				}
			}
			...
			if(item_type == 'standard') {
				if(site.settings.overselling != 1 && site.settings.product_expiry == 1){
					if(parseFloat(request_quantity) > parseFloat(cur_stock_qty)){
						$('#row_' + row_no).addClass('danger');
