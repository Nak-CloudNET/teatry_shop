...
$(document).on('click', '.qudel', function () {
     ...
     * Edit Row Modal Hanlder
     ...
     $(document).on('click', '.edit', function () {
        ...
        unit_price = formatMoney(row.children().children('.realuprice').val()),
			...
			if(item.options !== false) {
				...
				$.each(item.options, function () {
					if(o == 1) {
						...
						$('#piece').val((item.row.piece ? item.row.piece : 1));
				...
				$('#lbpiece').text("Piece");
				$('#piece').val((item.row.piece ? item.row.piece : 1));
				$('#wpiece').val(formatDecimal((item.row.wpiece ? item.row.wpiece : 1)));
			}
		}else{
...
function loadItems() {
    if (localStorage.getItem('quitems') ) {
		...
		/*if(type == "digital"){
			$.ajax({
				type: 'get',
				url:  site.base_url+'quotes/getProductNamesDigital/?id='+qid+'&ware='+wid+'&cid='+cid+'',
				dataType: "json",
				async:false,
				success: function (data) {
					localStorage.setItem('quitems',JSON.stringify(data));
				}
			});
		}*/
        ...
        $.each(quitems, function () {
			...
			*/
            ...
            if (item_tax_method == 0) {
                unit_price = formatMoney(real_unit_price);
            
            }else{
                unit_price = formatMoney(unit_price+item_discount);                
            }
			...
			*/
			...
			tr_html += '<td class="text-right"><input class="form-control input-sm text-right rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + formatDecimal(item_price) + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + formatMoney(unit_price) + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + formatMoney(unit_price) + '</span></td>';
 ...
 function add_invoice_item(item) {
    ...
    if (item == null) {
		...
		if (quitems[item_id]) {
			...
			localStorage.setItem('wid', item.warehouse_id);	
			localStorage.setItem('cid', item.customer_id);				
			localStorage.setItem('qid', item.row.id);
			localStorage.setItem('type', item.row.type);
		localStorage.setItem('quitems', JSON.stringify(quitems));
		loadItems();
    return true;
}
