	...
	if (ps == 'partial' || ps == 'paid') {
		if(paid_by != 'deposit') {
			if(ps == 'paid') {
				...
				if(paid > 0) {
					$('#amount_1').val(formatDecimal(parseFloat(paid)));
...
function loadItems() {
	if (localStorage.getItem('slitems')) {
		...
		$.each(slitems, function () {
			...
			if (site.settings.tax1 == 1) {
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						...
						if (item_tax_method == '0') {
							pr_tax_val = formatDecimal((((item.row.real_unit_price) * parseFloat(pr_tax.rate)) / (100 + parseFloat(pr_tax.rate))), 4);
							pr_tax_rate = formatDecimal(pr_tax.rate) + '%';
						} else {
							pr_tax_val = formatDecimal((((item.row.real_unit_price) * parseFloat(pr_tax.rate)) / 100), 4);
							pr_tax_rate = formatDecimal(pr_tax.rate) + '%';
						}
			...
			iprice = item_tax_method == 0 ? formatDecimal(item.row.real_unit_price-pr_tax_val, 4) : formatDecimal(item.row.real_unit_price);
			...
			*/
			...
			if (item_promotion == 1 && (current_date >= start_date && current_date <= end_date)){
				...
				tr_html += '<td class="text-right"><input class="form-control text-right rprice_t" name="net_price[]" type="text" id="price_' + row_no + '" value="' + iprice + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + unit_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + item.row.real_unit_price + '"> </td>';
			}
			...
			if (site.settings.product_discount == 1) {
				...
				tr_html += '<td class="text-right"><input class="text-right form-control rdiscount_t rdiscount" name="product_discount[]" type="text" id="discount_' + row_no + '" value="' + item_ds + '"><span style="display:none;" class="text-right sdiscount text-danger" id="sdiscount_' + row_no + '">' + formatMoney(item_discount * item_qty) + '</span></td>';
			}
			if (site.settings.tax1 == 1) {
		...
		var gtotal = parseFloat(((total - order_discount) + shipping) + invoice_tax);
