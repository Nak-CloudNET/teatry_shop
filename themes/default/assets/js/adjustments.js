...
function loadItems() {
    ...
    if (localStorage.getItem('qaitems')) {
        ...
        $.each(sortedItems, function () {
            ...
            var product_id = item.row.id, item_qty = item.row.qty, item_option = item.row.option, item_code = item.row.code, item_serial = item.row.serial, qoh = item.row.qoh, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;");
