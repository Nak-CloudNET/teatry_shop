		...
		$.each(slitems, function () {
				...
				if (pr_tax !== false) {
					if (pr_tax.type == 1) {
						...
						if (item_tax_method == '0') {
							...
							pr_tax_val = formatDecimal(((unit_price * item_qty - num_of_dis) * parseFloat(pr_tax.rate)) / (100));
							pr_tax_rate = formatDecimal(pr_tax.rate) + '%';
						} else {
							pr_tax_val = formatDecimal((((unit_price) * parseFloat(pr_tax.rate)) / 100) * item_qty);
							pr_tax_rate = formatDecimal(pr_tax.rate) + '%';
						}
			...
			*/
			...
			if (item_promotion == 1){
				...
				tr_html += '<td class="text-right"><span class="text-right ssubtotal" id="subtotal_' + row_no + '">' + formatMoney(item_price * item_qty - num_of_dis + pr_tax_val) + '</span><input type="hidden" name="grand_total[]" value="'+ formatMoney(item_price * item_qty - num_of_dis + pr_tax_val) +'"></td>';
			}
