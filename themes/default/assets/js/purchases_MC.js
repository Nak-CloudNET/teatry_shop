		...
		if(item.options !== false) {
			$.each(item.options, function () {
				if(this.id == opt && this.price != 0 && this.price != '' && this.price != null) {
					$('#pcost').val(this.price);
					$("#net_cost").text(formatMoney(this.price));
				}
			});
		}
	});
...
function loadItems() {
    ...
    if (localStorage.getItem('poitems')) {
        ...
        $.each(poitems, function () {
            ...
            tr_html += '<td class="text-right"><input class="form-control text-center net_cost" name="net_cost[]" type="text" id="cost_' + row_no + '" value="' + item_cost + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '"></td>';
            tr_html += '<td><input name="quantity_balance[]" type="hidden" class="rbqty" value="' + item_bqty + '"><input class="form-control text-center quantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
			tr_html += '<td class="text-right"><span class="text-right scost" id="scost_' + row_no + '">' + formatMoney(item_cost) + '</span></td>';
			
			/* Stock In Hand */
			tr_html += '<td class="text-right"><input class="form-control input-sm text-right rstock" name="rstock[]" type="hidden" id="stock_' + item.row.quantity + '" value="' + item.row.quantity + '"><input class="rstock" name="rstock[]" type="hidden" value="' + item.row.quantity + '"><input class="rstock" name="rstock[]" type="hidden" value="' + item.row.quantity + '"><span class="text-right scost" id="sstock_' + row_no + '">' + formatMoney(item.row.quantity) + '</span></td>';
            ...
            if (site.settings.tax1 == 1) {
                tr_html += '<td class="text-right"><input class="form-control input-sm text-right rproduct_tax" name="product_tax[]" type="hidden" id="product_tax_' + row_no + '" value="' + pr_tax.id + '"><span class="text-right sproduct_tax" id="sproduct_tax_' + row_no + '">' + (pr_tax_rate ? '(' + pr_tax_rate + ')' : '') + ' ' + formatMoney(pr_tax_val * item_qty) + '</span><input type="hidden" name="tax_percent[]" id="tax_percent" class="tax_percent" value="'+pr_tax_rate+'" /><input type="hidden" name="getTax[]" id="getTax" class="getTax" value="'+formatMoney(pr_tax_val * item_qty)+'" /></td>';
            }
            tr_html += '<td class="text-right"><span class="text-right ssubtotal get_total" id="subtotal_' + row_no + '">' + formatMoney(((parseFloat(item_cost) + parseFloat(pr_tax_val)) * parseFloat(item_qty))) + '</span></td>';
        ...
        var tfoot = '<tr id="tfoot" class="tfoot active"><th colspan="'+col+'">Total</th><th class="text-center">' + formatNumber(parseFloat(count) - 1) + '</th><th class="text-center"></th><th class="text-center"></th>';
        if (po_edit) {
...
$('.net_cost, .quantity').live('change',function(){
	...
	if(tax_per != ''){
		...
		row.find('.sproduct_tax').text('('+tax_per+') '+formatMoney(tax_pay));
	}
	var getTotal = formatMoney(((parseFloat(net_price) * parseFloat(quantity))  + parseFloat(tax_pay)));
