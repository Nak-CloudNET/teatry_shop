...
if (to_warehouse = localStorage.getItem('to_warehouse')) {
	...
	if(localStorage.getItem('received_cond') == 1){
		if (count > 1) {
 ...
 $(document).on('click', '.edit', function () {
    ...
    if(item.options !== false) {
        ...
        $.each(item.options, function () {
            ...
            $("<option />", {value: this.id, text: this.name, id: this.total_qty}).appendTo(opt);
            o++;
        });
    } 
 ...
 * Misc Actions
 ...
 function loadItems() {
    ...
    if (localStorage.getItem('toitems')) {
        ...
        product_tax = 0;
        $("#toTable tbody").empty();
        $('#add_transfer, #edit_transfer, #received_transfer').attr('disabled', false);
        ...
        $.each(toitems, function () {
            ...
            var product_id = item.row.id, item_type = item.row.type, combo_items = item.combo_items, item_cost = item.row.cost, item_qty = item.row.qty, item_bqty = item.row.quantity_balance, item_expiry = item.row.expiry, item_aqty = item.row.quantity, item_tax_method = item.row.tax_method, item_ds = item.row.discount, item_discount = 0, item_option = item.row.option, item_code = item.row.code, item_name = item.row.name.replace(/"/g, "&#034;").replace(/'/g, "&#039;"), item_serial = item.row.serial;
			item_qty
			
			/*KHR
			if(localStorage.getItem('received_cond')==1){
				item_qty = (item_qty - item_qty_received);
			}*/
		   ...
		   if(localStorage.getItem('hide_edit')==1){
				tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+'</span></td>';
            }else{
				tr_html = '<td><input name="product_id[]" type="hidden" class="rid" value="' + product_id + '"><input name="product_type[]" type="hidden" class="rtype" value="' + item_type + '"><input name="product_code[]" type="hidden" class="rcode" value="' + item_code + '"><input name="product_name[]" type="hidden" class="rname" value="' + item_name + '"><input name="product_option[]" type="hidden" class="roption" value="' + item_option + '"><span class="sname" id="name_' + row_no + '">' + item_name + ' (' + item_code + ')'+(sel_opt != '' ? ' ('+sel_opt+')' : '')+'</span> <i class="pull-right fa fa-edit tip tointer edit" id="' + row_no + '" data-item="' + item_id + '" title="Edit" style="cursor:pointer;"></i></td>';
			}
            ...
            tr_html += '<td class="text-right"><input class="form-control input-sm text-right rcost" name="net_cost[]" type="hidden" id="cost_' + row_no + '" value="' + formatDecimal(item_cost) + '"><input class="rucost" name="unit_cost[]" type="hidden" value="' + unit_cost + '"><input class="realucost" name="real_unit_cost[]" type="hidden" value="' + item.row.real_unit_cost + '"><span class="text-right scost" id="scost_' + row_no + '">';
            tr_html += '<input name="quantity_balance[]" type="hidden" class="rbqty" value="' + formatDecimal(item_bqty) + '"><input class="form-control text-center rquantity" name="quantity[]" type="text" value="' + formatDecimal(item_qty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();"></td>';
            if (site.settings.tax1 == 1) {
            ...
            tr_html += '<td class="text-center">' + item_aqty + '</td>';
            ...
            if (item.options !== false) {
                ...
                $('#row_' + row_no).addClass('danger');
                // if(site.settings.overselling != 1) { 
                    $('#add_transfer, #edit_transfer, #received_transfer').attr('disabled', true); 
                // }
            }
        ...
        tfoot += '<th class="text-center"></th><th class="text-center"><i class="fa fa-trash-o" style="opacity:0.5; filter:alpha(opacity=50);"></i></th></tr>';
