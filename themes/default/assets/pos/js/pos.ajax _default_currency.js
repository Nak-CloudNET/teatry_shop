...
$('#posdiscount').focus(function () {
		...
		if(site.settings.attributes == 1){
			if(item.options !== false) {
				...
				$.each(item.options, function () {
					...
					$("<option />", {value: this.id, text: this.name+" (" + formatDecimal(parseFloat(this.qty_unit)) + ")"}).appendTo(opt);
					o++;
				});
			}
			//console.log('console.log(item.all_group_price);'+item.all_group_price);
		}else{
			
			if(item.all_group_price !== false && item.all_group_price) {
				var gp = 1;
	...
	$(document).on('click', '#editItem', function () {
		...
		var price = parseFloat($('#pprice').val()) * parseFloat($('#own_rate').val());
...
function loadItems() {
	...
	if (localStorage.getItem('positems')) {
		...
		$.each(positems, function (i, e) {
			...
			var p_kh_rate 		= localStorage.getItem('exchange_kh'); 
			var default_price 	= 0;
			var default_rate 	= item.currency.rate;
			var us_rate 		= item.us_currency.rate;
			...
			var us = (real_unit_price/default_rate) * us_rate;
			var p_kh_price = us.toFixed(6);
			...
			if (item_promotion == 1){
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + p_kh_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + p_kh_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + p_kh_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + '$ ' + p_kh_price + '</span></td>';
			}else{
				tr_html += '<input class="rprice" name="net_price[]" type="hidden" id="price_' + row_no + '" value="' + p_kh_price + '"><input class="ruprice" name="unit_price[]" type="hidden" value="' + p_kh_price + '"><input class="realuprice" name="real_unit_price[]" type="hidden" value="' + p_kh_price + '"><span class="text-right sprice" id="sprice_' + row_no + '">' + '$ ' + p_kh_price + '</span></td>';
			}
			...
			tr_html += '<td class="text-right"><span class="price-kh">'+ real_unit_price +'</span></td>';
			...
			//tr_html += '<input class="form-control kb-pad text-center inhand" name="inhand[]" type="hidden" value="' + formatDecimal(item_aqty) + '" data-id="' + row_no + '" data-item="' + item_id + '" id="quantity_' + row_no + '" onClick="this.select();">';
		...
		if(postax2 = localStorage.getItem('postax2')){
		    var pta =postax2;  
		    if (pta == 2) {
				 total_tax = parseFloat(((total - order_discount) * (parseFloat(pr_tax.rate) / 100)));	
			}else if (pta == 1) {
				total_tax= 0;
			}
	    }
	...
	if(susp_id > 0){
		...
		$("#gtotal_khs").text('(៛ '+formatSA(parseFloat(gtotal_kh).toFixed(0)) + ')');
