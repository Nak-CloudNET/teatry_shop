                ...
                <div class="table-responsive">
                    <table class="table table-bordered table-condensed table-striped">
						...
						<?php 
							...
							if(count($sales) > 0){
								...
								$sql = "SELECT
										 ...
										 (
											CASE
											WHEN erp_sale_items.option_id > 0 THEN
													erp_sale_items.unit_cost * erp_product_variants.qty_unit
											ELSE
							...
							?>
								...
								<?php if($sale->type == 2 ){ ?>
									<tr style="font-weight:bold; color:red !important;">
										<td></td>
										<td colspan="9" class="info-reference_no right"><?= lang("subtotal")?> :</td>
										<td class="right"><?= $this->erp->formatMoney($total_costs * (-1)); ?></td>
										<td class="right"><?= $this->erp->formatMoney($amount * (-1)); ?></td>
										<td class="right"><?=$this->erp->formatMoney(($amount - $total_costs) * (-1))?></td>
									</tr>
								<?php }else{ ?>
									<tr style="font-weight:bold;">
										<td></td>
								...
								<?php 		
									...
									$g_total_costs += ($total_costs - $total_costs_return) - $total_costs_return;
									$g_total_prices += ($amount - $amount_return) - $amount_return;
                        ...
                        <tfoot>
							...
							<tr>
								<th colspan="10"  style="color:#0586ff"class="right info-foot"><?= lang("total_all") ?>  : </th>						
