<script type="text/javascript">
    ...
    $(document).ready(function () {
		...
		$('#plan').change(function () {
			var v = $("#plan option:selected").text();
            $('#modal-loading').show();
			if (v) {
                $.ajax({
                    type: "get",
	...
	<?php if ($items) {?>
		localStorage.setItem('usitems', JSON.stringify(<?=$items;?>));
		...
		localStorage.setItem('customer', '<?= $using_stock->customer_id; ?>');
					...
					<div class="col-md-4">			
						...
						<div class="form-group">
							...
							<?php
								...
								echo form_dropdown('plan', $pl, '', 'class="form-control"  id="plan" placeholder="' . lang("select") . ' ' . lang("plan") . '" style="width:100%; pointer-events:none;"')
                            ?>
						</div>
						<div class="form-group">
