...
<div class="modal-dialog">
    <div class="modal-content">
        ...
        echo form_open_multipart("sales/add_m_payment_loan/" . $loan->id, $attrib); ?>
			...
			<div class="table-responsive">
                <table id="CompTable" cellpadding="0" cellspacing="0" border="0"
					...
					if (!empty($combine_loan)) {
						...
						//$this->erp->print_arrays($combine_loan);
                        ...
                        foreach ($combine_loan as $combine_loans) { ?>
                            <tr class="row<?= $combine_loans->id ?>">
								...
								<td class="balance" val='<?=$combine_loans->payment?>'><?= $this->erp->formatDecimal(($combine_loans->payment-$combine_loans->paid_amount)); ?></td>
								...
								<td class="col-xs-3">
                                    ...
                                    <input type="text"   name="amount_paid_line[]" class="amount_paid_line form-control" value="<?= $this->erp->formatDecimal(($combine_loans->payment-$combine_loans->paid_amount)); ?>">
                                </td>
                        ...
                        <?php 
						$total_interest += $combine_loans->interest;
						$total_prin  	+= $combine_loans->principle;
						$total       	+= ($combine_loans->payment-$combine_loans->paid_amount);
                        $tamountpaid 	+= ($combine_loans->payment-$combine_loans->paid_amount);
						}
                    } else {
                                ...
                                <div class="payment">
                                    <div class="form-group">
                                        ...
                                        <input name="interest-paid" type="text" id="interest"
                                               value="<?= $this->erp->formatDecimal($total_interest); ?>"
                                               class="pa form-control kb-pad interest" required="required" readonly />
										<input name="interest" type="hidden" value="<?= $this->erp->formatDecimal($total_interest); ?>" />	   
                                ...
                                <div class="payment">
                                    <div class="form-group">
                                        ...
                                        <input name="amount-paid" type="text" id="principle"
                                               value="<?= $this->erp->formatDecimal($total_prin); ?>"
                                               class="pa form-control kb-pad amount" required="required" readonly />
										<input name="principle_amt" id="principle_amt" type="hidden" value="<?=$this->erp->formatDecimal($total_prin)?>">
										<input name="loan_id" type="hidden" value="<?=$id?>"  />
										<input name="paid_amount" type="hidden" value="<?=$total?>" />
										<input name="principle" type="hidden" value="<?=$total_prin?>" />
