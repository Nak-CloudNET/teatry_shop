...
<body>
	<div class="container" style="width: 575px;">
		<div class="col-xs-12" style="width: 559px;margin-left: -3% !important;" class="con1">
			<div class="row" style="margin-top: 20px !important;">
		
				<div class="col-sm-3 col-xs-3">
				<?php if(!empty($biller->logo)) { ?>
					<img src="<?= base_url() ?>assets/uploads/logos/<?= $biller->logo; ?>" style="width: 135px !important; margin-left: 25px;" id="logo"/>
				<?php } ?>
				</div>
				...
				<center>
					...
					<?php if(!empty($biller->vat_no)) { ?>
						<p style="font-size: 9px !important;">លេខអត្តសញ្ញាណកម្ម អតប (VAT No):&nbsp;<?= $biller->vat_no; ?></p>
					<?php } ?>
					
					<?php if(!empty($biller->address)) { ?>
						<p style="margin-top:-10px !important;font-size: 9px !important;">អាសយដ្ឋាន ៖ &nbsp;<?= $biller->address; ?></p>
					<?php } ?>
					
					<?php if(!empty($biller->phone)) { ?>
						<p style="margin-top:-10px !important;font-size: 9px !important;">ទូរស័ព្ទលេខ (Tel):&nbsp;<?= $biller->phone; ?></p>
					<?php } ?>
					
					<?php if(!empty($biller->email)) { ?>
						<p style="margin-top:-10px !important;font-size: 9px !important;">សារអេឡិចត្រូនិច (E-mail):&nbsp;<?= $biller->email; ?></p>
					<?php } ?>
				</center>
				</div>
				<div class="col-sm-3 col-xs-3">
			...
			<div class="row">
				...
				<div class="col-sm-5 col-xs-5">
					<table style="font-size: 11px;">
						<tr>
							<td style="width: 35%;">លេខរៀងវិក្កយបត្រ </br> Invoice No.</td>
							<td style="width: 10%;">:</td>
							<td style="width: 55%;"><?= $invs->reference_no ?></td>
						</tr>
						<tr>
			...
			<div class="row">
				<div class="col-sm-12 col-xs-12">
					<table class="table table-bordered" style="margin-top: 10px;width: 100%;margin-left:-1px;">
						<tbody style="font-size: 9px;">
							<tr class="thead" style="background-color: #444 !important; color: #FFF !important;">
								<th>ល.រ<br /><?= strtoupper(lang('no')) ?></th>
								<th>បរិយាយមុខ<br>ទំនិញ<br /><?= strtoupper(lang('description')) ?></th>
								<th>ការបញ្ជាក់ពីមុខ<br>ទំនិញ<br /><?= strtoupper(lang('specification')) ?></th>
								...
								<?php } ?>
								<th>តម្លៃសរុបតាម<br>មុខទំនិញ<br /><?= strtoupper(lang('subtotal')) ?></th>
							</tr>
							<?php 
								$no = 1;
								foreach ($rows as $row) {
			...
			<?php if($invs->note){ ?>
			<div style="border-radius: 5px 5px 5px 5px;border:1px solid black;font-size: 10px !important;margin-top: 10px;height: auto;width:100%;" id="note" class="col-md-12 col-xs-12">
				<p style="margin-left: 10px;margin-top:10px;"><?php echo strip_tags($invs->note); ?></p>
			</div>
			<?php } ?>
			<br><br><br>
			<div id="footer" class="row" style="margin-top: 80px !important;max-width: 106% !important;">
				<div class="col-sm-4 col-xs-4">
					<hr style="margin:0; border:1px solid #000;">
					<center>
						<p style="font-size: 10px;">ហត្ថលេខា និងឈ្មោះអ្នករៀបចំ</p>
						<p style="margin-top:-10px;font-size: 10px !important;">Prepared's Signature & Name</p>
					</center>
				</div>
				<div class="col-sm-4 col-xs-4">
					<hr style="margin:0; border:1px solid #000;">
					<center>
						...
						<p style="margin-top:-10px;font-size: 10px !important;">Seller's Signature & Name</p>
					</center>
				</div>
				<div class="col-sm-4 col-xs-4">
					<hr style="margin:0; border:1px solid #000;">
					<center>
						...
						<p style="margin-top:-10px;font-size: 10px !important;">Customer's Signature & Name</p>
					</center>
				</div>
			</div>
			<footer></footer>
