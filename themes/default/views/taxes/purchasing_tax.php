            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                for (var i = 0; i < aaData.length; i++) {
					...
					if(aaData[aiDisplay[i]][9]!=null){
						totald += parseFloat(aaData[aiDisplay[i]][9]);
					}
					if(aaData[aiDisplay[i]][10]!=null){
                    paidd += parseFloat(aaData[aiDisplay[i]][10]);
					}
					if(aaData[aiDisplay[i]][11]!=null){
                    balanced += parseFloat(aaData[aiDisplay[i]][11]);
					}
                }
                ...
                nCells[8].innerHTML = currencyFormat(balance);
				 nCells[9].innerHTML = currencyFormat(totald);
                nCells[10].innerHTML = currencyFormat(paidd);
                nCells[11].innerHTML = currencyFormat(balanced);
            }
...
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i
                class="fa-fw fa fa-star"></i><?=lang('purchasing_tax') . ' (' . ($warehouse_id ? $warehouse->name : lang('all_warehouses')) . ')';?>
        </h2>
		<div class="box-icon">
            <ul class="btn-tasks">
                <li class="dropdown">
        ...
        <div class="row">
            <div class="col-lg-12">
				...
				<div id="form">
                    ...
                    <?php echo form_open("taxes/purchasing_tax"); ?>
                                        <div class="row">
						<div class="col-md-4">
							<div class="form-group">
								<input <?php if ($this->input->post('hide_tax')){echo 'checked';}?> type="checkbox" id="hdie_tax" class="form-control" name="hide_tax" value="1">
								<span>Show all</span>
							</div>
						</div>				
...
<script type="text/javascript" charset="utf-8">
		...
		$('#hide_tax').click(function (e) {
            e.preventDefault();
            $('#form_action').val('hide_tax');
            $('#action-form').submit();
