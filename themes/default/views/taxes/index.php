<script>
    $(document).ready(function () {
        var oTable = $('#SupData').dataTable({
			...
			{column_number: 3, filter_default_label: "[<?=lang('account_name');?>]", filter_type: "text", data: []},
            {column_number: 4, filter_default_label: "[<?=lang('account_section');?>]", filter_type: "text", data: []},
        ], "footer");
    });
</script>
<?php if ($Owner) {
    echo form_open('taxes/taxes_actions', 'id="action-form"');
} ?>
<div class="box">
    <div class="box-header">
        <h2 class="blue"><i class="fa-fw fa fa-users"></i><?= lang('list_ac_head_tax'); ?></h2>
            ...
            <ul class="btn-tasks"> 
                <li class="dropdown">
                    ...
                    <ul class="dropdown-menu pull-right" class="tasks-menus" role="menu" aria-labelledby="dLabel">
                        <li><a href="<?= site_url('taxes/add'); ?>" data-toggle="modal" data-target="#myModal"
                               id="add"><i class="fa fa-plus-circle"></i> <?= lang("add_chart_account_taxes"); ?></a></li>
						<?php if ($Owner || $Admin) { ?>
							<li><a href="<?= site_url('taxes/import_taxes_csv'); ?>" data-toggle="modal"
								   data-target="#myModal"><i class="fa fa-plus-circle"></i> <?= lang("add_chart_account_taxes_csv"); ?>
								</a></li>
							<li>
						...
						<?php }?>				
                        <li class="divider"></li>
                        <li><a href="#" class="bpo" title="<?= $this->lang->line("delete_chart_taxes") ?>"
                               data-content="<p><?= lang('r_u_sure') ?></p><button type='button' class='btn btn-danger' id='delete' data-action='delete'><?= lang('i_m_sure') ?></a> <button class='btn bpo-close'><?= lang('no') ?></button>"
                               data-html="true" data-placement="left"><i
                                    class="fa fa-trash-o"></i> <?= lang('delete_chart_taxes') ?></a></li>
                    </ul>
                </li>
            </ul>
        </div>
                    ...
                    <table id="SupData" cellpadding="0" cellspacing="0" border="0"
                        ...
                        <thead>
							<tr class="primary">
								...
								<th style="width:20%;"><?= lang("account_name_kh"); ?></th>
