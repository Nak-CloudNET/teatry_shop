...
<script>
    $(document).ready(function () {
        var oTable = $('#POData').dataTable({
            ...
            'sAjaxSource': '<?=site_url('taxes/getLargePurchases' . ($warehouse_id ? '/' . $warehouse_id : '')).'/?v=1'.$v?>',
            ...
            "aoColumns": [{
                "bSortable": false,
                "mRender": checkbox
            }, {"mRender": fld}, null,null, null, null, {"mRender": row_status}, {"mRender": currencyFormat}, {"mRender": currencyFormat}, {"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender": currencyFormat},{"mRender": currencyFormat},null, {"mRender": row_status_confirm}],
            ...
            "fnFooterCallback": function (nRow, aaData, iStart, iEnd, aiDisplay) {
                ...
                var totald = 0, paidd = 0, balanced = 0;
                for (var i = 0; i < aaData.length; i++) {
					...
					if((aaData[aiDisplay[i]][10])!=null){
						totald += parseFloat(aaData[aiDisplay[i]][10]);
					}
					if((aaData[aiDisplay[i]][11])!=null){
                    paidd += parseFloat(aaData[aiDisplay[i]][11]);
					}
					if((aaData[aiDisplay[i]][12])!=null){
                    balanced += parseFloat(aaData[aiDisplay[i]][12]);
					}
                }
                ...
                nCells[9].innerHTML = currencyFormat(balance);
				 nCells[10].innerHTML = currencyFormat(totald);
                nCells[11].innerHTML = currencyFormat(paidd);
                nCells[12].innerHTML = currencyFormat(balanced);
            }
        }).fnSetFilteringDelay().dtFilter([
            {column_number: 1, filter_default_label: "[<?=lang('date');?> (yyyy-mm-dd)]", filter_type: "text", data: []},
            {column_number: 2, filter_default_label: "[<?=lang('ref_no');?>]", filter_type: "text", data: []},
            {column_number: 3, filter_default_label: "[<?=lang('inv_no');?>]", filter_type: "text", data: []},
            ...
            {column_number: 10, filter_default_label: "[<?=lang('remark');?>]", filter_type: "text", data: []},
        ], "footer");
...
<script type="text/javascript" charset="utf-8">
        $(document).ready(function() {
            $('body').on('click', '#purchase_tax', function() {
                ...
                $('#myModal').modal({remote: '<?=base_url('taxes/add_large_purchasing_tax_form');?>?data=' + arrItems + ''});
                $('#myModal').modal('show');
            });
