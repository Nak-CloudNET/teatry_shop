...
class Sales_model extends CI_Model
{
    ...
    public function updateSale($id, $data, $items = array())
    {
        ...
        if ($this->db->update('sales', $data, array('id' => $id))) {
			...
			foreach($sale_items as $sItem){
				$this->db->delete("inventory_valuation_details", array("field_id" => $sItem->id, "type"=> "SALE"));
				$this->db->delete("purchase_items",array("transaction_id"=>$sItem->id, "transaction_type"=>"SALE"));
			}
  ...
  //           ->group_by('deliveries.id')
	...
	public function getPOById($sale_id, $status)
    {
        $this->db->select('po');
		if($status == 'invoice'){
			...
			$q = $this->db->get_where('sale_order', array('id' => $sale_id), 1);
			if ($q->num_rows() > 0) {
				return $q->row();
			}
		}
