...
class Sales extends MY_Controller
{
	...
	function invoice_return($id = NULL)
    {
        ...
        $this->load->view($this->theme.'sales/invoice_return', $this->data);
    }
	...
	function getSales($warehouse_id = NULL)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
				->select("sales.id, sales.date as date,erp_quotes.reference_no as q_no, sale_order.reference_no as so_no, 
							sales.reference_no as sale_no, sales.po, sales.biller, sales.customer, 
			...
			$this->datatables
				->select("sales.id, sales.date as date,erp_quotes.reference_no as q_no, sale_order.reference_no as so_no, 
							sales.reference_no as sale_no, sales.po, sales.customer, companies.company, 
							users.username AS saleman, sales.sale_status, COALESCE(erp_sales.grand_total,0) as amount,
							COALESCE((SELECT SUM(erp_return_sales.grand_total) FROM erp_return_sales WHERE erp_return_sales.sale_id = erp_sales.id), 0) as return_sale,
							COALESCE( (SELECT SUM(IF((erp_payments.paid_by != 'deposit' AND ISNULL(erp_payments.return_id)), erp_payments.amount, IF(NOT ISNULL(erp_payments.return_id), ((-1)*erp_payments.amount), 0))) FROM erp_payments WHERE erp_payments.sale_id = erp_sales.id),0) as paid, 
							(SELECT SUM(IF(erp_payments.paid_by = 'deposit', erp_payments.amount, 0)) FROM erp_payments WHERE erp_payments.sale_id = erp_sales.id  ) as deposit,
		...
		if ($po) {
			$this->datatables->where('sales.po', $po);
		}
		if ($payment_status) {
...
AND "2018-03-31 23:59:00"';
        ...
        if ($this->form_validation->run() == true) {
            ...
            if (!empty($_POST['val'])) {
				...
				if ($this->input->post('form_action') == 'print_statement_with_logo') {
					foreach ($_POST['val'] as $id) {
                        $sale = $this->site->getReceivableByID($id);
					}
                    $this->print_statement_header($_POST['val'], $sale);
                }
				if ($this->input->post('form_action') == 'print_statement_without_logo') {
					foreach ($_POST['val'] as $id) {
                        $sale = $this->site->getReceivableByID($id);
					}
                    $this->print_statement_no_header($_POST['val'], $sale);
                }
                ...
                if ($this->input->post('form_action') == 'export_excel' || $this->input->post('form_action') == 'export_pdf') {
                    ...
                    if ($this->input->post('form_action') == 'export_pdf') {
						...
						$this->excel->getActiveSheet()->getStyle('A3:M3')->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
						$this->excel->getActiveSheet()->getStyle('A1:M1')->getFont()->setBold(true)
                                ->setSize(15);
						$rw = 4;
						foreach ($_POST['val'] as $id) {
							$this->excel->getActiveSheet()->getStyle("A" . $rw . ":M" . $rw)->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_MEDIUM);
    ...
    function add($sale_order_id = NULL, $delivery_id = NULL,$quote_ID = NULL)
    {
        ...
        if ($this->form_validation->run() == true) {
			...
			if($invoice_view == 0){
				redirect("sales/invoice_a5/".$s->row()->id);
			}
			...
			if ($delivery_id){
				...
				$this->data['po'] = $this->sales_model->getPOById($sale_order->sale_id);		
    ...
    function edit($id = NULL)
    {
        ...
        if ($this->form_validation->run() == true && $this->sales_model->updateSale($id, $data, $products,$payment, (isset($loans)? $loans : ""))) {
			$this->session->set_userdata('remove_s2', '1');
			...
			$redirect = $this->sales_model->getSalesById($id);
			if($redirect->pos == 1){
				redirect("pos/sales");
			}else{
				redirect("sales");
	...
	function getSales_items($start = NULL, $end = NULL)
    {
        ...
        if ($biller_id) {
            $this->datatables
				...
				->join($sl_items, 'sl_items.sale_id = sales.id', 'inner')
				->join($dl_items, 'dl_items.sale_id = sales.id', 'inner')
