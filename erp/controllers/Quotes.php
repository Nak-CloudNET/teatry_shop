...
class Quotes extends MY_Controller
{
    ...
    public function getQuotes($warehouse_id = null)
    {
        ...
        if ($warehouse_id) {
            $this->datatables
                ->select("quotes.id, quotes.date, quotes.reference_no, po, quotes.biller, quotes.customer,users.username, quotes.grand_total, quotes.issue_invoice,  quotes.status")
                ->from('quotes')
            ...
            $this->datatables
                ->select("quotes.id, quotes.date, quotes.reference_no, po, quotes.biller, IF(erp_companies.company = '',erp_quotes.customer, erp_companies.company) AS customer,users.username, quotes.grand_total, quotes.issue_invoice,  quotes.status")
                ->from('quotes')
                ->join('payments', 'payments.deposit_quote_id = quotes.id', 'left')
		...
		if ($po) {
			$this->datatables->where('quotes.po', $po);
		}
		if ($biller) {
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run() == true) {
            ...
            $po = $this->input->post('po') ? $this->input->post('po') : '';
	...
	function invoice_quotes($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        $this->erp->checkPermissions('add', true, 'sales');
	...
	function invoice_quotes_combo($id = NULL, $view = NULL, $save_bufffer = NULL)
    {
        if ($this->input->get('id')) {
            $id = $this->input->get('id');
        }
        ...
        $this->data['quote_items'] = $this->quotes_model->getQuoteItemsData($id);
		$this->data['qid'] = $id;
		$this->load->view($this->theme . 'quotes/invoice_quotes_combo.php', $this->data);
    }
